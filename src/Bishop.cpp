...
Bishop::Bishop(const string& name, const string& coor, bool isWhite, bool isFirstMove, SDL_Renderer * renderer) : Piece(name, coor, isWhite, isFirstMove)
{
    texture = NULL;
    setTexture(renderer);
}
...
vector<Square> Bishop::getPossibleMoves(const Board &b)
{
...
void Bishop::setTexture(SDL_Renderer * renderer)
{
    ...
    texture_rect_crop.x = 215;  //the x coordinate
...
SDL_Texture * Bishop::getTexture() const
{
    return texture;
}
